        ...
        <label class="weui_cell weui_check_label" for="{{$cart_item->product->id}}">
            ...
            <div class="weui_cell_bd weui_cell_primary">
              <div style="position: relative;">
                <img class="bk_preview" src="http://localhost/laravel-v5.1.11/public{{$cart_item->product->preview}}" class="m3_preview" onclick="_toProduct({{$cart_item->product->id}});"/>
...
<script type="text/javascript">
  ...
  function _onDelete() {
    var product_ids_arr = [];
    $('input:checkbox[name=cart_item]').each(function(index, el) {//遍历里面的
      if($(this).attr('checked') == 'checked') {//选中的放进数组
        product_ids_arr.push($(this).attr('id'));//选中的放进数
      }
    });
